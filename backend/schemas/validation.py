...
class MarketDataSchema(BaseModel):
    symbol: str = Field(regex=r'^[A-Z]{2,10}USDT?$', description="Symbol must be valid trading pair")
...
class SentimentDataSchema(BaseModel):
    ...
    classification: str = Field(regex=r'^(BEARISH|NEUTRAL|BULLISH)$')
    timestamp: datetime
...
class NotificationSettingsSchema(BaseModel):
    ...
    chat_id: Optional[str] = Field(regex=r'^-?\d+$')
