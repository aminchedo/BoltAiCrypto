export class SentimentAPI {
  ...
  async getFearGreedIndex(): Promise<any> {
    try {
      const response = await fetch(`${this.apis.fearGreed}?limit=1&format=json`);
  ...
  async getCoinMarketCapSentiment(symbol: string): Promise<any> {
    try {
      const response = await fetch(
        `${this.apis.coinMarketCap}/cryptocurrency/quotes/latest?symbol=${symbol}&convert=USD`,
        {
          headers: {
            'X-CMC_PRO_API_KEY': this.keys.coinMarketCap
          }
        }
      );
      const data = await response.json();
  ...
  async getCryptoCompareSentiment(symbol: string): Promise<any> {
    try {
      ...
      const response = await fetch(
        `${this.apis.cryptoCompare}/social/coin/general?coinId=${fsym}&api_key=${this.keys.cryptoCompare}`
      );
      const data = await response.json();
